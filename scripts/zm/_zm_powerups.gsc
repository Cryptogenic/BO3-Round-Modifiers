...
function get_valid_powerup()
{
	...
	while( 1 )
	{
		if( powerup == "insta_kill" && level.modifiers_drop_noinstakill )
		{
			powerup = get_next_powerup();
			continue;
		}

		if( powerup == "fire_sale" && level.disable_firesale_drop )
		{
			powerup = get_next_powerup();
			continue;
		}
...
{
	if(level.modifiers_joker_active == true)
		return true;
...
{
	if (level.modifiers_dropzone_round == true)
	{
		level.powerup_drop_count = 0;
	}
	...
	else if (level.modifiers_dropzone_round == true && rand_drop < 20)
	{
		debug = "zm_bgb_power_vacuum";
	}
	else if (rand_drop > 2)
	...
	if(level.modifiers_high_drop)
	{
		powerup = zm_net::network_safe_spawn( "powerup", 1, "script_model", drop_point + (0,0,200));
	}
	else
	{
...
function specific_powerup_drop( powerup_name, drop_spot, powerup_team, powerup_location, pickup_delay, powerup_player, b_stay_forever )
{
	if(level.modifiers_high_drop)
	{
		powerup = zm_net::network_safe_spawn( "powerup", 1, "script_model", drop_spot + (0,0,200));
	}
	else
	{
