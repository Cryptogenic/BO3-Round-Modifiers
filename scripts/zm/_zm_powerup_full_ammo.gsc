...
function __init__()
{
	...
	{
		zm_powerups::add_zombie_powerup( "full_ammo", "p7_zm_power_up_max_ammo", &"ZOMBIE_POWERUP_MAX_AMMO", &zm_powerups::func_should_always_drop, !POWERUP_ONLY_AFFECTS_GRABBER, !POWERUP_ANY_TEAM, !POWERUP_ZOMBIE_GRABBABLE );
	}
}
...
function full_ammo_powerup( drop_item , player)
{
	...
	for (i = 0; i < players.size; i++)
	{
		...
		for( x = 0; x < primary_weapons.size; x++ )
		{
			...
			{
				if(level.modifiers_joker_active && RandomInt(100) < 50)
				{
					players[i] setweaponammostock(primary_weapons[x], 0);
					players[i] setweaponammoclip(primary_weapons[x], 0);
				}
				else
				{
